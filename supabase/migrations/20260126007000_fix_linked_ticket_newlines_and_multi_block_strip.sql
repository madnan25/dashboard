-- Fix linked ticket sync:
-- 1) Use real newlines (previous version accidentally inserted literal "\n\n")
-- 2) Strip repeated header blocks reliably, including ones affected by the "\n" literal bug
...
BEGIN
  ...
  -- Undo a prior bug where we inserted literal "\n" sequences.
  tail := replace(tail, E'\\n', E'\n');
  ...
  -- Unwrapped header blocks: remove ALL occurrences, even if back-to-back.
  tail := regexp_replace(
    tail,
