-- Fix linked design ticket header de-duplication:
-- Prior versions (including some app-created tickets) can contain a blank line before "Subtask details:",
-- which caused the legacy header-strip regex to miss and accumulate duplicates.
...
BEGIN
  ...
  -- Legacy unwrapped blocks (handle optional blank line before "Subtask details:")
  tail := regexp_replace(
    tail,
    '(?s)\\s*Design work for:.*?\\nParent ticket:.*?\\nSubtask:.*?\\n(?:\\s*\\n)?Subtask details:.*?(\\n\\n|$)',
    '',
