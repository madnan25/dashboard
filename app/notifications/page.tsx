...
export default async function NotificationsPage() {
  ...
  let initialItems: Notification[] = [];
  let unreadCount = 0;

  if (profile && isMarketingTeamProfile(profile)) {
    const [items, count] = await Promise.all([
      repo.listNotifications({ userId: profile.id, limit: 50 }),
      repo.countUnreadNotifications(profile.id)
    ]);
    initialItems = items;
    unreadCount = count;
  }
    ...
    <main className="min-h-screen px-4 md:px-6 pb-10">
      <div className="mx-auto w-full max-w-5xl space-y-6">
        ...
        {profile && isMarketingTeamProfile(profile) ? (
          <NotificationsCenter userId={profile.id} initialItems={initialItems} initialUnreadCount={unreadCount} />
        ) : (
